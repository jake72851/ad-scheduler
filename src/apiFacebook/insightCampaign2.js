...
exports.request = async function (userId, accountId, campaignId, pageId) {
  ...
  const params = {
    ...
    time_range: { since: dayjs().format('YYYY-MM-DD'), until: dayjs().format('YYYY-MM-DD') },
    filtering: [{ field: 'ad.funding_page_id', value: pageId, operator: 'EQUAL' }],
  };
  // const insights = await account.getInsights(fields, params);
  let insights = await new Campaign(campaignId).getInsights(fields, params);
  console.log('insights campaign2.length =', insights.length);
  ...
  if (result.length > 0) {
    await userFacebook.updateOne(
      { apiId: 'campaign2', campaignId, createdDay: data.createdDay },
      { $set: data },
      { upsert: true }
    );
  }
