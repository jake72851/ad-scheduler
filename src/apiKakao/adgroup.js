...
const waitTime = 10000; // 이 API는 광고계정 번호, 앱 ID당 10초에 한 번으로 요청이 제한

exports.request = async function (accessToken, userId, accountId, accountName, campaignId, campaignName, startDate, endDate, adGroupIdsArr) {
  console.log(' ');
  ...
  for (const itemDimension of CONF.KAKAO.DIMENSION) {
    for (const itemMetrics of CONF.KAKAO.METRICS) {
      ...
      const report = await axios.request('GET', CONF.KAKAO.API + CONF.KAKAO.API_ADGROUP, param, '', header);
