...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  console.log('ad dimension =', dimension);
  // ID dimensions >
  // 'advertiser_id',
  // 'campaign_id',
  // 'adgroup_id',
  ...
  const metricBasic = [
    ...
    'app_promotion_type',
    'adgroup_name', // Supported at Ad Group and Ad level.
  ...
  if (dimensionJoin === 'country_code' || dimensionJoin === 'ad_id, country_code') {
    metric = [...CONF.TIKTOK.METRICS_COUNTRY_CODE];
  } else if (dimensionJoin === 'ad_type' || dimensionJoin === 'ad_id, ad_type') {
    metric = [...CONF.TIKTOK.METRICS_AD_TYPE];
  } else if (dimensionJoin === 'custom_event_type' || dimensionJoin === 'ad_id, custom_event_type') {
    metric = [...CONF.TIKTOK.METRICS_CUSTOM_EVENT_TYPE];
  } else if (dimensionJoin === 'image_id' || dimensionJoin === 'ad_id, image_id') {
    metric = [...CONF.TIKTOK.METRICS_IMAGE_ID];
  } else {
    metric = [...metricBasic];
  }
  // console.log('adgroup metric =', metric);
  ...
  // console.log('ad 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('ad ' + dimensionJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('ad ' + dimensionJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('ad 인사이트 result list =', result.data.data.list);
    console.log('ad ' + dimensionJoin + ' 인사이트 result list.length =', result.data.data.list.length);

    let arr = [...result.data.data.list];
    let breakdown = null;
    if (dimensionJoin !== 'ad_id') {
    ...
    for (let i = 1; i < result.data.data.page_info.total_page; i++) {
      ...
      if (arr.length > 0) {
        console.log('ad breakdown =', breakdown);
    ...
    // console.log('arr =', arr);
    // console.log('ad 페이징 처리후 결과 arr.length =', arr.length);
