...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  const datas = {
    ...
    metrics: [
      ...
      // 'user_action', // adgroup 지원안함 - For the VIDEO_RELATED scene, available values include WATCHED_TO_END, LIKED, COMMENTED, and SHARED. For the CREATOR_RELATED scene, available values are FOLLOWING and VIEW_HOMEPAGE
      ...
      // 'frequency', // adgroup 지원안함
      // 'cost_per_1000_reached', // adgroup 지원안함
      // 'reach', // adgroup 지원안함
  ...
  console.log('adgroup AUDIENCE 인사이트 datas.dimensions =', datas.dimensions);
  ...
  // console.log('adgroup AUDIENCE 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('adgroup AUDIENCE ' + dimensionAddJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('adgroup AUDIENCE ' + dimensionAddJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('adgroup AUDIENCE ' + dimensionAddJoin + ' 인사이트 result list =', result.data.data.list);
    console.log('adgroup AUDIENCE ' + dimensionAddJoin + ' 인사이트 result list.length =', result.data.data.list.length);
