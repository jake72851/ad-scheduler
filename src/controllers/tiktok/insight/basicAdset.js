...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  console.log('adgroup dimension =', dimension);
  // ID dimensions >
  // 'advertiser_id',
  // 'campaign_id',
  // 'adgroup_id',
  ...
  if (dimensionJoin === 'country_code' || dimensionJoin === 'adgroup_id, country_code') {
    metric = [...CONF.TIKTOK.METRICS_COUNTRY_CODE];
  } else if (dimensionJoin === 'ad_type' || dimensionJoin === 'adgroup_id, ad_type') {
    metric = [...CONF.TIKTOK.METRICS_AD_TYPE];
  } else if (dimensionJoin === 'custom_event_type' || dimensionJoin === 'adgroup_id, custom_event_type') {
    metric = [...CONF.TIKTOK.METRICS_CUSTOM_EVENT_TYPE];
  } else if (dimensionJoin === 'image_id' || dimensionJoin === 'adgroup_id, image_id') {
  ...
  const datas = {
    ...
    data_level: 'AUCTION_ADGROUP', // ['AUCTION_ADVERTISER', 'RESERVATION_AD', 'RESERVATION_ADGROUP', 'AUCTION_ADGROUP', 'RESERVATION_CAMPAIGN', 'AUCTION_AD', 'AUCTION_CAMPAIGN', 'RESERVATION_ADVERTISER']
  ...
  // console.log('adgroup 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('adgroup ' + dimensionJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('adgroup ' + dimensionJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('adgroup 인사이트 result list =', result.data.data.list);
    console.log('adgroup ' + dimensionJoin + ' 인사이트 result list.length =', result.data.data.list.length);
