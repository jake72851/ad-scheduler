...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  const metricBasic = [
    ...
    'ctr',
    'reach',
    // 'cost_per_1000_reached', // country_code 지원안함
    'frequency',
    'currency',
    ...
    'video_views_p75',
    'video_views_p100',
    // 'average_video_play', // country_code 지원안함
    // 'average_video_play_per_user', // country_code 지원안함
    ...
    // 'clicks_on_hashtag_challenge', // country_code 지원안함 - 캠페인 기간 동안 광고의 해시태그 챌린지에 대한 총 클릭 수입니다.
    // 'engagements', // 캠광고계정페인 지원안함
    // 'engagement_rate', // 광고계정 지원안함
    // 'skip_ad', // country_code 지원안함

    // Additional format metrics > // country_code 지원안함
  ...
  if (dimensionJoin === 'country_code' || dimensionJoin === 'advertiser_id, country_code') {
    metric = [...metricBasic];
  } else {
    metric = [...metricBasic];
    metric.push(
  ...
  const datas = {
    ...
    data_level: 'RESERVATION_ADVERTISER', // ['AUCTION_ADVERTISER', 'RESERVATION_AD', 'RESERVATION_ADGROUP', 'AUCTION_ADGROUP', 'RESERVATION_CAMPAIGN', 'AUCTION_AD', 'AUCTION_CAMPAIGN', 'RESERVATION_ADVERTISER']
  ...
  // console.log('광고계정 RESERVATION 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('광고계정 RESERVATION ' + dimensionJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('광고계정 RESERVATION ' + dimensionJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('광고계정 RESERVATION ' + dimensionJoin + ' 인사이트 result list =', result.data.data.list);
    console.log('광고계정 RESERVATION ' + dimensionJoin + ' 인사이트 result list.length =', result.data.data.list.length);
    ...
    // console.log('arr =', arr);
    console.log('광고계정 RESERVATION ' + dimensionJoin + ' arr.length =', arr.length);
