...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  const datas = {
    ...
    metrics: [
      // Attribute metrics >
      // 'campaign_name', // 광고계정 지원안함
      // 'campaign_id', // Supported at Ad Group and Ad level.
      // 'objective_type', // 광고계정 지원안함
      // 'split_test', // 광고계정 지원안함
      // 'campaign_budget', // 광고계정 지원안함
      // 'campaign_dedicate_type', // 광고계정 지원안함
      // 'adgroup_name',
      // 'adgroup_id',
  ...
  console.log('광고계정 AUDIENCE 인사이트 datas.dimensions =', datas.dimensions);
  ...
  // console.log('광고계정 AUDIENCE 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('광고계정 AUDIENCE ' + dimensionAddJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('광고계정 AUDIENCE ' + dimensionAddJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('캠페인 AUDIENCE ' + dimensionAddJoin + ' 인사이트 result list =', result.data.data.list);
    console.log('광고계정 AUDIENCE ' + dimensionAddJoin + ' 인사이트 result list.length =', result.data.data.list.length);
