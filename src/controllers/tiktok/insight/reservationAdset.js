...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  if (dimensionJoin !== 'country_code') {
    metric = [...metricBasic];
    metric.push('campaign_id', 'adgroup_name');
  } else {
    metric = [...metricBasic];
  }
  ...
  const datas = {
    ...
    data_level: 'RESERVATION_ADGROUP', // ['AUCTION_ADVERTISER', 'RESERVATION_AD', 'RESERVATION_ADGROUP', 'AUCTION_ADGROUP', 'RESERVATION_CAMPAIGN', 'AUCTION_AD', 'AUCTION_CAMPAIGN', 'RESERVATION_ADVERTISER']
  ...
  // console.log('adGroups RESERVATION 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('adGroups RESERVATION ' + dimensionJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('adGroups RESERVATION ' + dimensionJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('adGroups RESERVATION ' + dimensionJoin + ' 인사이트 result list =', result.data.data.list);
    console.log('adGroups RESERVATION ' + dimensionJoin + ' 인사이트 result list.length =', result.data.data.list.length);
    ...
    // console.log('arr =', arr);
    console.log('adGroups RESERVATION ' + dimensionJoin + ' arr.length =', arr.length);
