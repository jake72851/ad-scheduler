...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  const datas = {
    ...
    data_level: 'AUCTION_AD', // ['AUCTION_ADVERTISER', 'RESERVATION_AD', 'RESERVATION_ADGROUP', 'AUCTION_ADGROUP', 'RESERVATION_CAMPAIGN', 'AUCTION_AD', 'AUCTION_CAMPAIGN', 'RESERVATION_ADVERTISER']
    ...
    metrics: [
      ...
      'adgroup_name',
      'adgroup_id', // Supported at Ad level.
      ...
      // 'user_action', // ad 지원안함 - For the VIDEO_RELATED scene, available values include WATCHED_TO_END, LIKED, COMMENTED, and SHARED. For the CREATOR_RELATED scene, available values are FOLLOWING and VIEW_HOMEPAGE
      ...
      // 'frequency', // ad 지원안함
      // 'cost_per_1000_reached', // ad 지원안함
      // 'reach', // ad 지원안함
      // 'live_play_count', // adgroup 지원안함
    ],
  ...
  console.log('ad AUDIENCE 인사이트 datas.dimensions =', datas.dimensions);
  ...
  // console.log('ad AUDIENCE 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('ad AUDIENCE ' + dimensionAddJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('ad AUDIENCE ' + dimensionAddJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('ad AUDIENCE ' + dimensionAddJoin + ' 인사이트 result list =', result.data.data.list);
    console.log('ad AUDIENCE ' + dimensionAddJoin + ' 인사이트 result list.length =', result.data.data.list.length);

    let arr = [...result.data.data.list];
    // console.log('arr =', arr);

    console.log('페이징 시작시간 =', dayjs().format('YYYY-MM-DD HH:mm:ss'));
    for (let i = 1; i < result.data.data.page_info.total_page; i++) {
      console.log('page num =', i + 1);
      datas.page = i + 1;

      // const arraySize = Buffer.byteLength(JSON.stringify(arr), 'utf8') / 1024 / 1024;
      // console.log('array size in MB:', arraySize);
      // console.log('arr =', arr);
      // console.log('순서정렬 시작시간 =', dayjs().format('YYYY-MM-DD HH:mm:ss'));
      // arr.sort((a, b) => a.dimensions.ad_id - b.dimensions.ad_id);
      // console.log('순서정렬 종료시간 =', dayjs().format('YYYY-MM-DD HH:mm:ss'));
      // console.log('arr =', arr);
      ...
      if (arr.length > 0) {
        ...
        // 용량문제가 있는 경우가 있어 개별로 저장 처리함
        // for (const arrayItem of arr) {
        // console.log('arrayItem.dimensions.ad_id =', arrayItem.dimensions.ad_id);
        ...
        const data = {
          ...
          page: i,
          // adId: arrayItem.dimensions.ad_id,
          serviceType: 'AUCTION',
      ...
      // const arraySize = Buffer.byteLength(JSON.stringify(result.data.data.list), 'utf8') / 1024 / 1024;
      // console.log('array size in MB:', arraySize);
      // arr.push(...result.data.data.list);
      arr = [...result.data.data.list];
    }
    // console.log('arr =', arr);
    console.log('페이징 종료시간 =', dayjs().format('YYYY-MM-DD HH:mm:ss'));
  }
};
