...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  // console.log('ad DSA 인사이트 result =', result);

  const dimensionJoin = dimensionAdd.join(', ');

  if (result.data.message !== 'OK') {
    console.log('ad DSA ' + dimensionJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('ad DSA ' + dimensionJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('ad DSA ' + dimensionJoin + ' 인사이트 result list =', result.data.data.list);
    console.log('ad DSA ' + dimensionJoin + ' 인사이트 result list.length =', result.data.data.list.length);

    let arr = [...result.data.data.list];
    for (let i = 1; i < result.data.data.page_info.total_page; i++) {
      console.log('page num =', i + 1);
      datas.page = i + 1;
      console.log('ad DSA ', dimensionJoin, ' arr.length =', arr.length);
