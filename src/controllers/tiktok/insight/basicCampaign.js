...
exports.request = async function (userId, advertiserIds, advertiserName, termDay, dimensionAdd) {
  ...
  } else if (dimensionJoin === 'campaign_id, country_code') {
    metric = [...CONF.TIKTOK.METRICS_COUNTRY_CODE];
  ...
  } else if (dimensionJoin === 'campaign_id, ad_type') {
    metric = [...CONF.TIKTOK.METRICS_AD_TYPE];
    metric.push('campaign_name', 'campaign_budget', 'split_test', 'campaign_dedicate_type');
  } else if (dimensionJoin === 'custom_event_type') {
    metric = [...CONF.TIKTOK.METRICS_CUSTOM_EVENT_TYPE];
  } else if (dimensionJoin === 'campaign_id, custom_event_type') {
    metric = [...CONF.TIKTOK.METRICS_CUSTOM_EVENT_TYPE];
    metric.push('campaign_name', 'campaign_budget', 'split_test', 'campaign_dedicate_type');
  } else if (dimensionJoin === 'image_id') {
    metric = [...CONF.TIKTOK.METRICS_IMAGE_ID];
  } else if (dimensionJoin === 'campaign_id, image_id') {
    metric = [...CONF.TIKTOK.METRICS_IMAGE_ID];
    metric.push('campaign_name', 'campaign_budget', 'split_test', 'campaign_dedicate_type');
  } else {
    metric = [...metricBasic];
  }
  // console.log('캠페인 metric =', metric);
  ...
  // console.log('캠페인 인사이트 result =', result);

  if (result.data.message !== 'OK') {
    console.log('캠페인 ' + dimensionJoin + ' 인사이트 err =', result.data.message);
  } else {
    console.log('캠페인 ' + dimensionJoin + ' 인사이트 result page_info =', result.data.data.page_info);
    // console.log('캠페인 인사이트 result list =', result.data.data.list);
    console.log('캠페인 ' + dimensionJoin + ' 인사이트 result list.length =', result.data.data.list.length);
