...
exports.auth = async function (userId) {
  ...
  const authInfoList = await adAccountsInfo.find({ user_id: userId, auth_type: 'kakao', status: 1 }).toArray();
  console.log('kakao > authInfoList =', authInfoList);
  /*
    [
      {
  ...
  if (authInfoList.length === 0) {
    ...
    */
    ...
    /*
      ...
      console.log('api_result =', api_result)
    */
    authInfo = { type: 'kakao', access_token: authInfoList[0].access_token, user_id: userId };
  } else if (authInfoList.length > 1) {
